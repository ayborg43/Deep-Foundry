...
const kinds: Integration["kind"][] = ["email", "calendar", "slack", "discord", "github", "twitter", "webhook"];

const kindLabels: Partial<Record<Integration["kind"], string>> = { github: "GitHub", twitter: "Twitter / X" };

export default function IntegrationsPage() {
  const router = useRouter(); const [workspaceId, setWorkspaceId] = useState(""); const [rows, setRows] = useState<Integration[]>([]);
  const [kind, setKind] = useState<Integration["kind"]>("webhook"); const [name, setName] = useState(""); const [endpoint, setEndpoint] = useState(""); const [secret, setSecret] = useState("");
  ...
  return <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-6 px-4 py-10"><header><h1 className="flex items-center gap-2 text-2xl font-semibold"><CableIcon className="size-6" />Integrations</h1><p className="text-sm text-muted-foreground">Connect events from email, calendars, chat, GitHub, or signed webhooks to workflows.</p></header>{error && <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}
    ...
    <Card><CardHeader><CardTitle>Connect a service</CardTitle></CardHeader><CardContent><form onSubmit={create} className="grid gap-4 sm:grid-cols-2"><div className="grid gap-1.5"><Label htmlFor="integration-kind">Service</Label><Select value={kind} onValueChange={(value) => setKind(value as Integration["kind"])}><SelectTrigger id="integration-kind"><SelectValue /></SelectTrigger><SelectContent>{kinds.map((item) => <SelectItem key={item} value={item}>{kindLabels[item] ?? item[0].toUpperCase() + item.slice(1)}</SelectItem>)}</SelectContent></Select></div><div className="grid gap-1.5"><Label htmlFor="integration-name">Display name</Label><Input id="integration-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Production GitHub" /></div><div className="grid gap-1.5 sm:col-span-2"><Label htmlFor="integration-endpoint">Endpoint URL (optional)</Label><Input id="integration-endpoint" type="url" value={endpoint} onChange={(e) => setEndpoint(e.target.value)} placeholder="https://hooks.example.com/..." /></div><div className="grid gap-1.5 sm:col-span-2"><Label htmlFor="integration-secret">API token / secret (optional)</Label><Input id="integration-secret" type="password" value={secret} onChange={(e) => setSecret(e.target.value)} placeholder={kind === "twitter" ? "OAuth 2.0 user access token from the X developer portal" : "Sent as a Bearer token; generated for you if left blank"} /><p className="text-xs text-muted-foreground">{kind === "twitter" ? "Required for Twitter / X: paste an OAuth 2.0 user access token with the tweet.write scope." : "Optional — a random signing secret is generated when left blank."}</p></div><Button className="w-fit" disabled={busy || !workspaceId}><PlusIcon data-icon="inline-start" />Connect</Button></form></CardContent></Card>
