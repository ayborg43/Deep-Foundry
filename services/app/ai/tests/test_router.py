...
class CapabilityNegotiationTests(RouterTestBase):
    ...
    def test_allows_tools_against_tool_calling_model(self):
        self.adapter.responses["deepseek-chat"] = NormalizedResponse(
...
class SuccessLoggingTests(RouterTestBase):
    def test_success_logs_model_call_with_usage_and_cost(self):
        ...
        self.router.generate([], [], ModelConfig(model_id="deepseek-chat"))
        ...
        self.assertEqual(call.model_id, "deepseek-chat")
...
class FallbackTests(RouterTestBase):
    def test_falls_back_on_adapter_error_and_logs_fallback_used(self):
        self.adapter.responses["deepseek-reasoner"] = AdapterError("degraded")
        self.adapter.responses["deepseek-chat"] = NormalizedResponse(
            content="fallback worked", usage=Usage(input_tokens=5, output_tokens=5)
        )

        response = self.router.generate(
            [], [], ModelConfig(model_id="deepseek-reasoner"), fallback_model_id="deepseek-chat"
        )

        self.assertEqual(response.content, "fallback worked")
        self.assertEqual(self.adapter.calls, ["deepseek-reasoner", "deepseek-chat"])
    ...
    def test_falls_back_on_rate_limit(self):
        self.adapter.responses["deepseek-reasoner"] = RateLimitedError("slow down")
        self.adapter.responses["deepseek-chat"] = NormalizedResponse(content="ok", usage=None)
    ...
    def test_no_fallback_configured_raises_and_logs_error(self):
        self.adapter.responses["deepseek-reasoner"] = AdapterError("down")
        with self.assertRaises(AdapterError):
            self.router.generate([], [], ModelConfig(model_id="deepseek-reasoner"))
        call = ModelCall.objects.get()
        self.assertEqual(call.status, ModelCall.Status.ERROR)

    def test_fallback_also_fails_raises_original_chained(self):
        self.adapter.responses["deepseek-reasoner"] = AdapterError("primary down")
        self.adapter.responses["deepseek-chat"] = AdapterError("fallback also down")
...
class StreamingTests(RouterTestBase):
    def test_stream_yields_chunks_and_logs_after_completion(self):
        self.adapter.stream_chunks["deepseek-chat"] = [
            StreamChunk(delta="Hel"),
            StreamChunk(
                delta="lo", finish_reason="stop", usage=Usage(input_tokens=2, output_tokens=2)
            ),
        ]
        stream_config = ModelConfig(model_id="deepseek-chat", stream=True)
    ...
    def test_stream_error_logs_failure_and_reraises(self):
        self.adapter.stream_chunks["deepseek-chat"] = AdapterError("stream broke")
        stream_config = ModelConfig(model_id="deepseek-chat", stream=True)
        with self.assertRaises(AdapterError):
    ...
    def test_generate_forces_non_streaming_even_if_config_says_stream(self):
        self.adapter.responses["deepseek-chat"] = NormalizedResponse(content="sync", usage=None)
        response = self.router.generate(
